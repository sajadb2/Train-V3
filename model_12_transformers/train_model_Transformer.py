...
class GPTConfig:
    ...
    vocab_size: int = 50176 # number of tokens: 50,000 BPE merges + 256 bytes tokens + 1 <|endoftext|> token
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        config_args = {
            'gpt2':         dict(n_layer=12, n_head=8, n_embd=768),  # 124M params
            'gpt2-medium':  dict(n_layer=24, n_head=16, n_embd=1024), # 350M params
            'gpt2-large':   dict(n_layer=36, n_head=20, n_embd=1280), # 774M params
            'gpt2-xl':      dict(n_layer=48, n_head=25, n_embd=1600), # 1558M params
        }[model_type]
        config_args['vocab_size'] = 50176 # always 50257 for GPT model checkpoints
...
for i in range(50):
    ...
    # Check if loss is approaching the target
    if loss.item() <= 0.09:
